...
export default function Resetpassword() {
  ...
  const handleClick = async (e) => {
    e.preventDefault();
    await fetch(`${process.env.REACT_APP_BACK_END_URL}user/reset/password?${code}`, { method: "PUT", headers: { 'Content-Type': "application/JSON" }, body: JSON.stringify({ password: password }) }).then((data) => {
